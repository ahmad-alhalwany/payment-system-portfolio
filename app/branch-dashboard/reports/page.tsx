...
import BranchReportsGuidePanel from "@/components/branch-manager/ReportsGuidePanel";
import { reportsApi, type ChartData, type ReportStats } from "@/app/api/reports";
import { useLocale } from "@/components/providers/LocaleProvider";
import { getStatusBadgeClass, translateActivityStatus } from "@/lib/dashboard-utils";

type BranchReportTab = "transactions" | "employees" | "daily" | "charts";
...
export default function BranchReportsPage() {
  const { t } = useLocale();
  const mr = t.dashboard.manager.reports;
  const r = t.dashboard.reports;
  const today = new Date().toISOString().slice(0, 10);

  const [branchName, setBranchName] = useState("");
  const [tab, setTab] = useState<BranchReportTab>("transactions");
  ...
  useEffect(() => {
    reportsApi.branches({}).then((res) => {
      const first = res.data.branch_stats?.[0];
      if (first && typeof first.name === "string") setBranchName(first.name);
    }).catch(() => {});
  }, []);

  const tabs: { key: BranchReportTab; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
    { key: "transactions", label: mr.tabs.transactions, icon: FileText },
    { key: "employees", label: mr.tabs.employees, icon: Users },
    { key: "daily", label: mr.tabs.daily, icon: Calendar },
    { key: "charts", label: mr.tabs.charts, icon: BarChart3 },
  ];
  ...
  const buildParams = useCallback(() => {
    ...
    if (tab === "employees") params.employee_role = "employee";
    if (search.trim()) params.search = search.trim();
    return params;
  }, [tab, fromDate, toDate, today, status, transferType, currency, employeeStatus, search]);
  ...
  const handleTabChange = (next: BranchReportTab) => {
    setTab(next);
    setGenerated(false);
    setRows([]);
    setStats(null);
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `branch-report-${tab}-${today}.csv`;
    a.click();
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <div>
        <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white">{mr.title}</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">{mr.subtitle}</p>
        {branchName && (
          <p className="flex items-center gap-1.5 text-sm text-primary-600 dark:text-primary-400 mt-2 font-medium">
            <Building2 className="w-4 h-4" />
            {mr.branchLabel}: {branchName}
          </p>
        )}
      </div>
          ...
          <div className="rounded-2xl border border-slate-200/80 dark:border-white/10 bg-white dark:bg-slate-900/50 p-5 md:p-6 shadow-sm">
            ...
            >
              ...
              {(tab === "transactions" || tab === "charts") && (
                <>
                  <Field label={mr.filters.searchTransfers}>
              ...
              {tab === "employees" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Field label={mr.filters.searchEmployees}>
                    <input className={inputClass} value={search} onChange={(e) => setSearch(e.target.value)} />
                  </Field>
                </div>
              )}
