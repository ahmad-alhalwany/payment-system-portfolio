...
export default function PrintTransferView({ transfer, onClose }: PrintTransferViewProps) {
  ...
  const generateAndShareImage = async (service: 'whatsapp' | 'telegram') => {
    try {
      ...
      // تحويل HTML إلى canvas مع إعدادات محسّنة (استخدام scale مناسب للحصول على جودة جيدة وحجم صغير)
      const canvas = await html2canvas(receiptElement, {
        scale: 2, // استخدام scale 2 للحصول على جودة أفضل في الصورة
        useCORS: true,
        allowTaint: false,
        logging: false,
      ...
      // تحويل canvas إلى صورة JPEG بجودة 0.85 للحصول على حجم صغير (عادة 200-500 KB)
      canvas.toBlob(async (blob) => {
        ...
        const fileName = `receipt-${transfer.id}.jpg`;
      ...
      }, 'image/jpeg', 0.85); // JPEG بجودة 85% للحصول على حجم صغير
    } catch (error: any) {
      console.error('Error generating image:', error);
      alert(`حدث خطأ أثناء إنشاء الصورة: ${error?.message || 'خطأ غير معروف'}\nيرجى المحاولة مرة أخرى.`);
    }
  };
    ...
    >
      ...
      <div className="flex justify-center gap-3 mt-2 mb-2 print:hidden sticky bottom-0 bg-white/90 z-20 py-2 border-t border-gray-200 flex-wrap">
        ...
        <button
          ...
          onClick={() => generateAndShareImage('telegram')}
