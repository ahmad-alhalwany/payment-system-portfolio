...
import { Eye, Printer, RefreshCw, ArrowUpRight } from "lucide-react";
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-12 text-center">
                  <ArrowUpRight className="w-10 h-10 mx-auto text-slate-300 dark:text-slate-600 mb-2" />
...
function IconBtn({ icon: Icon, title, onClick }: { icon: React.ComponentType<{ className?: string }>; title: string; onClick: () => void }) {
  return (
    <button type="button" title={title} onClick={onClick} className="p-1.5 rounded-lg text-slate-500 hover:text-primary-600 hover:bg-primary-500/10">
